...
# # ── Function: Anonymise sensitive columns ────────────────────────────────────
...
#         df["Data Scientist"] = df["Data Scientist"].map(ds_map).fillna("Unknown DS")
...
#         df["Client"] = df["Client"].map(client_map).fillna("Unknown Client")
#         print(f"🔒 Anonymised 'Client': {len(client_map)} unique values mapped.")

#     if "Use case" in df.columns:
#         df["Use case"] = df["Client"].fillna("") + " - " + df["Data Scientist"].fillna("")

#     return df


# # ── Helper: Repeat colors if needed ──────────────────────────────────────────
...
#         # Step 5: Anonymise sensitive columns
...
#         # ── Chart 1: Count of Use Cases by Client (Bar) ───────────────────────
#         client_counts = df["Client"].value_counts().reset_index()
#         client_counts.columns = ["Client", "Count"]
#         client_counts["Count"] = client_counts["Count"].astype(int)
...
#         fig_client_donut = go.Figure(data=[go.Pie(
#             labels=final_counts.index,
#             values=final_counts.values,
#             hole=0.5,
#             marker=dict(colors=get_colors(len(final_counts))),
#             textinfo="label+percent",
#             hoverinfo="label+value+percent"
#         )])
#         fig_client_donut.update_layout(
#             title="Use Cases Distribution by Client (Top 10 + Others)",
#             paper_bgcolor="white",
#             title_font=dict(size=16),
#             showlegend=True
#         )
...
#             hole=0.5,
#             marker=dict(colors=get_colors(len(status_counts))),
#             textinfo="label+percent",
#             hoverinfo="label+value+percent"
#         )])
#         fig_status_donut.update_layout(
#             title="DS Project Status Distribution",
#             paper_bgcolor="white",
#             title_font=dict(size=16),
#             showlegend=True
#         )
...
#         # ── Build tech counter (used for Chart 4 AND Chart 5) ─────────────────
#         raw_tech_list = []
#         for row in df["Core Technology"]:
#             words = [
#                 w.strip() for w in row.split(",")
#                 if w.strip() and w.strip() not in exclude_words_tech
#             ]
#             raw_tech_list.extend(words)

#         tech_counter = Counter(raw_tech_list)  # ← used in Chart 5

#         # Build DataFrame for Chart 4
#         df_tech = pd.DataFrame(
#             tech_counter.items(), columns=["Technology", "Count"]
#         ).sort_values("Count", ascending=False).head(15)
#         df_tech["Count"] = df_tech["Count"].astype(int)
...
#             tech_counter.items(), columns=["Core Technology", "Count"]
#         ).sort_values("Count", ascending=False)

#         top_20       = df_tech_all.head(20)
#         others_tech  = df_tech_all.iloc[20:]["Count"].sum()

#         df_tech_donut = pd.concat([
#             top_20,
#             pd.DataFrame([{"Core Technology": "Others", "Count": others_tech}])
#         ], ignore_index=True)
#         df_tech_donut = df_tech_donut[df_tech_donut["Count"] > 0]

#         fig_tech_donut = go.Figure(data=[go.Pie(
#             labels=df_tech_donut["Core Technology"],
#             values=df_tech_donut["Count"],
#             hole=0.5,
#             marker=dict(colors=get_colors(len(df_tech_donut))),
#             textinfo="label+percent",
#             hoverinfo="label+value+percent"
#         )])
#         fig_tech_donut.update_layout(
#             title="Top Core Technologies Distribution",
#             paper_bgcolor="white",
#             title_font=dict(size=16),
#             showlegend=True
#         )
...
#                 domain_counts.items(), columns=["Business Domain", "Count"]
#             )
...
#         stacked_data = (
#             stacked_data[stacked_data["Total"] > 0]
...
#                 text=stacked_data[col].apply(lambda v: str(int(v)) if v > 0 else ""),
#                 textposition="inside"
#             ))
#         fig_stacked.update_layout(
#             barmode="stack",
#             title="Client vs Project Status (Stacked)",
#             xaxis_title="Client",
#             yaxis_title="Number of Use Cases",
#             plot_bgcolor="white",
#             paper_bgcolor="white",
#             title_font=dict(size=16),
#             xaxis=dict(tickangle=-45),
...
#             df_final_sun = df_final_sun[
#                 df_final_sun["Level 1"].notna() & (df_final_sun["Level 1"] != "")
#             ]
...
#             df_sun_counts = (
#                 df_final_sun.groupby(["Level 1", "Level 2"])
#                 .size()
...
#                 values="Count",
#                 title="Hierarchical Algorithm Segments",
#                 color="Level 1",
...
#     const dsIdx     = columns.indexOf("Data Scientist");

#     function applyFilters() {{
#         const statusVal = document.getElementById("filter-status").value;
#         const clientVal = document.getElementById("filter-client").value;
#         const dsVal     = document.getElementById("filter-ds").value;
...
#             const matchDS     = dsVal === "All" || (dsIdx >= 0 && row[dsIdx] === dsVal);
...
#         document.getElementById("kpi-clients").innerText = new Set(filtered.map(r => r[clientIdx])).size;
#     }}
...
if __name__ == "__main__":
    ...
    INPUT_CSV = "data_science_projects_latest.csv"

    # Step 1: Load and clean the pre-anonymised CSV
    ...
    tech_counter = Counter(raw_tech_list)
